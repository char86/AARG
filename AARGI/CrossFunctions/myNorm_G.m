...
if nargin<3; opt=[]; end; 
if numel(opt)<1; opt=[-1 1 1]; end; 
if numel(opt)<6; opt(6)=0; end
if opt(1)<1||opt(1)>nn; dim=nn; else dim=opt(1); end

if dim~=nn; 
    pp=1:nn; 
    pp(dim)=nn; pp(nn)=dim; 
    dd=permute(dd,pp); AA=permute(AA,pp); 
end;
dd=reshape(dd,[],ss0(dim)); 
AA=reshape(AA,[],4); smm=mean(AA);  
smm=smm(1); 
mm=size(dd,1); 
Ao=zeros(mm,ss0(dim)); %Ao=zeros(ss0(dim),mm);
if opt(2)==0; 
    smm=opt(4); 
end
...
for ii=1:mm;
    ...
    if opt(3)==0; 
        if mod (ii,llh)==1;  
            waitbar(floor(ii/llh)/10,hh, ['Normalizing... ' num2str(ii) '/' num2str(mm)]); 
        end; 
    end
end;
if opt(3)==0; 
